...
const thumbsUp = document.querySelector(
  ".running-score-container .alert-success"
);
const thumbsDown = document.querySelector(
  ".running-score-container .alert-danger"
);
//photo gallery
...
let nextClassfellow = function (objectArray, studentIndex) {
  ...
  let tempAnswer;
  let tempIndices = [];
  // pull out the current student beforehand
  let tempNamesArray = objectArray.filter(
    (student, idx) => idx !== studentIndex
  );
  ...
  quizAnswers.forEach((answer) => {
    ...
    tempAnswerId = fnRandomizeNumber(0, tempNamesArray.length - 1);
    

    const thisAnswer = tempNamesArray.splice(tempAnswerId, 1)[0];
    console.log(thisAnswer);
    
    answer.innerText = thisAnswer.name;
...
quizForm.addEventListener("submit", (e) => {
  ...
  getLabel = document
    .getElementById(rightOptionID)
    .parentElement.querySelector(".quiz-option-label");
